...
class block:
    def __init__(self, entry, block_num, prev_hash, miner_public_key, driver_private_keys, genesis=False):
        self.entry = entry
    ...
    def get_block_hash(self):
        string_to_hash = self.previous_block_hash + str(self.nonce) + str(self.block_num)
        if not self.genesis:
            string_to_hash += self.entry.hash
        return hash_string(string_to_hash).hexdigest()
    
    def generate_miner_signature(self, miner_private_key):
        string_to_sign = self.entry.hash + str(self.block_num) + self.previous_block_hash
    ...
    def verify_miner_signature(self):
        if not self.entry.verify_entry_signature():
            return False
        data = self.entry.hash + str(self.block_num) + self.previous_block_hash
